...
def run_training(
    ...
    # CLI > Config > Heuristic
    resolved_input_size = input_size or int(model_cfg.get("input_size", 5 * horizon if horizon > 1 else 10))
    resolved_max_steps = int(max_steps if max_steps is not None else model_cfg.get("max_steps", 100))
    resolved_learning_rate = float(learning_rate if learning_rate is not None else model_cfg.get("learning_rate", 1e-3))
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", type=str, default="configs/wfv_config.json", help="Path to JSON/YAML config file")
    parser.add_argument("--max-lag", type=int, default=5, help="Max lag order used during feature engineering")
    parser.add_argument("--input-size", type=int, default=None, help="N-BEATS input_size override")
    parser.add_argument("--max-steps", type=int, default=None, help="Max training steps override")
